...
- **Authentication**: Optional Google sign-in via Firebase Authentication with a custom Blazor `AuthenticationStateProvider`. All routes remain public; Firestore stores per-account preferences protected by owner-only security rules.
...
- **Transport Data (GTFS-Realtime)**:
  ...
  - Route-lazy-loads `MyTransportAppWASM.Gtfs` when the map is opened; the startup path does not download GTFS code.
  - Uses a bounded selective Protocol Buffers wire parser that projects only the vehicle fields consumed by the map. `Google.Protobuf` is retained only in tests and host benchmarks for parity validation.
- **Weather Data**:
...
- **Multi-layered Forecasts**:
  1. _Baseline_: Current weather conditions (temperature, precipitation, probability of rain).
  2. _Radar_: Next-hour rain radar prediction.
  3. _Outlooks_: Long-term daily forecasts.
...
- Firebase Web App configuration is stored in the `Firebase` section of `wwwroot/appsettings.json`, with optional `appsettings.{Environment}.json` overrides matching the active Blazor environment. Deployment can populate the base `Firebase` section from the `FIREBASE_CONFIG_JSON` repository variable.
