...
public static class ProtocolBufferUtils
{
      ...
      where T : class, IMessage<T>, new()
  {
    ...
    try
    {
      byte[] buffer = System.Buffers.ArrayPool<byte>.Shared.Rent(81920); // 80KB initial
      ...
      try
      {
        while (true)
        {
          ...
          int bytesRead = await stream.ReadAsync(buffer.AsMemory(totalBytesRead), cancellationToken).ConfigureAwait(false);
          if (bytesRead == 0) break;
          totalBytesRead += bytesRead;
        }

        var parser = GetParser<T>();
        // Yield control before parsing large messages to keep UI responsive
        if (totalBytesRead > 4096)
        {
            await Task.Yield();
        }
        cancellationToken.ThrowIfCancellationRequested();
        return parser.ParseFrom(buffer.AsSpan(0, totalBytesRead));
      }
      finally
      {
        System.Buffers.ArrayPool<byte>.Shared.Return(buffer);
